...
const PLATFORMS = {
  'linux-x64': 'ncs-context-engine-linux-x64',
  'linux-arm64': 'ncs-context-engine-linux-arm64',
  'darwin-arm64': 'ncs-context-engine-darwin-arm64',
  'win32-x64': 'ncs-context-engine-win32-x64',
}
...
try {
  ...
  console.error(
    ...
    `Try reinstalling: npm install -g ncs-context-engine`
  )
  process.exit(1)
}
...
// `npx ncs-context-engine@latest` cannot replace the cached binary and npm
...
child.on('error', (err) => {
  console.error(`Failed to launch ncs-context-engine binary: ${err.message}`)
  process.exit(1)
})
