...
- [x] 2.3 proto, vue grammars; Angular/Next.js/SvelteKit resolvers
  - Vue: bounded `.vue` `<script>`/`<script setup>` extraction reuses JS/TS grammars with line offsets; template AST intentionally unsupported. Focused parser tests pass.
  - Proto: vendored `vendor/tree-sitter-protobuf` (crates.io `devgen-tree-sitter-protobuf` 0.21.0 grammar, Imotai MIT; generated parser LANGUAGE_VERSION 13, runtime-verified under tree-sitter 0.25). `.proto` detection plus package/message/enum/service/rpc/import symbols and `Imports` edges. Focused tests pass.
  - Resolvers: Angular (`@angular/core`/`@angular/common`, decorator-to-class edges), Next.js (`next` + app/pages route files only, non-route app files rejected), SvelteKit (`@sveltejs/kit`, route `load`/`actions`/page edges). Focused suites pass.
  - README en/vi/zh updated to 23 languages including Vue and Protocol Buffers.
...
Phase 1 complete except 1.1's latency gate (implemented, default-off, host-blocked: no `~/.context-engine`/embedding keys here, `ab_bench.sh` is Windows-hardcoded). Phase 2: 2.1 complete (`trace-path`, `symbol-context`, `impact`), 2.2 complete (Ollama native HTTP and local ONNX provider), 2.3 complete (proto + Vue parsing; Angular/Next.js/SvelteKit resolvers), 2.4 complete (`export-graph` CLI). ONNX runtime proof uses a test-only generated fixture covering reordered named inputs, optional `token_type_ids`, and `Cast`/`Add`/`Unsqueeze`/`Concat`; broader model/operator compatibility remains outside that fixture's scope. Phase 2 remaining only the 1.1 latency gate.
