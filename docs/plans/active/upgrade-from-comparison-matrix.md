...
- [ ] 2.2 Local/ONNX embedding provider + Ollama provider and docs
  - Ollama PARTIAL 2026-09-08: native `/api/embed` transport supports keyed and no-key setups, strict `Provider::parse`, `embed`/`embed_query` no longer assume API keys, and mock transport tests prove both `Authorization` branches. Proof: `cargo test --lib embedding::voyage` 14 passed, `git diff --check` clean. ONNX remains PENDING: local named-input inference and wiring are not implemented, `onnx` is not an accepted provider, and no completion/commit claim applies to 2.2.
