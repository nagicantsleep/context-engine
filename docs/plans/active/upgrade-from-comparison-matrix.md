...
- [ ] 2.2 Local/ONNX embedding provider + Ollama provider and docs
  - Ollama implementation: native `/api/embed`, keyed/no-key auth, strict provider parsing, provider-aware indexing/query construction, and cache namespace. Proof: `cargo test --lib embedding::voyage` 14 passed; `cargo check -q`; `git diff --check`.
  - ONNX implementation: provider-neutral client, tract-onnx/tokenizers loader, named `input_ids`/`attention_mask`/optional `token_type_ids`, dynamic-axis output validation, masked mean pooling, L2 normalization, model+tokenizer+contract fingerprint identity, config migration v14→v15, and factory wiring. Unit proof: `cargo test --lib embedding::onnx` 3 passed. End-to-end ONNX inference remains UNVERIFIED because the repository has no compatible model/tokenizer fixture and tract-onnx does not provide a serializer; do not mark 2.2 complete until a real fixture is supplied and exercised.
