    ...
    pub async fn embed_batch(
        ...
        if self.inner.provider == super::Provider::Ollama {
            let key = self
                ...
                .unwrap_or("");
            return self
                .try_embed_with_key(key, texts, input_type)
                .await
                .map_err(|e| match e {
                    super::retry::EmbedError::RateLimited => {
                        anyhow::anyhow!("embedding rate limited")
                    }
                    super::retry::EmbedError::Transient(err)
                    | super::retry::EmbedError::Other(err) => err,
                });
        }
